...
def handler(event, context):
    ...
    sourceS3Bucket = s3event['Records'][0]['s3']['bucket']['name']
    sourceS3Key = s3event['Records'][0]['s3']['object']['key']
    ...
    try:
        ...
        # Get Default job settings in the lambda zip file in the current working directory
        with open('mediaconvertjob.json') as json_data:
        ...
        for j in jobs:
            ...
            # path
            destinationS3 = 's3://' + os.environ['DestinationBucket'] + '/' \
                + os.environ['DestinationBucketPrefix'] + '/' \
                + os.path.splitext(os.path.basename(sourceS3Key))[0]


            for outputGroup in jobSettings['OutputGroups']:
                logger.info("Transcoding type: %s", outputGroup['OutputGroupSettings']['Type'])
